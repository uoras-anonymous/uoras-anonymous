    ...
    while True:
        if epoch % 10 == 0:
            ...
            for s in np.random.permutation(range(NUM_VD_SAMPLES)):
                ...
                for evec_vd in evecs_vd.T:
                    ...
                    # Predict p and q
                    featvct_vd = torch.cat([etavct_vd, hvctx_vd], dim=0).to(torch.float64)
                    with torch.no_grad():
        ...
        for s in np.random.permutation(range(NUM_TRAIN_SAMPLES)):
            ...
            for k in np.random.permutation(range(batch_size)):
                ...
                # Predict p and q
                featvct_tr = torch.cat([etavct_tr, hvctx_tr], dim=0).to(device).to(torch.float64)
